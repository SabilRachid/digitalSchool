...
public interface ProfessorDashboardRepository extends JpaRepository<Classe, Long> {
        ...
        @Query("SELECT c.name, AVG(ss.grade) " +
        ...
        @Query("SELECT CASE " +
                "WHEN ss.grade BETWEEN 0 AND 5 THEN '0-5' " +
                "WHEN ss.grade BETWEEN 5 AND 8 THEN '5-8' " +
                "WHEN ss.grade BETWEEN 8 AND 10 THEN '8-10' " +
                "WHEN ss.grade BETWEEN 10 AND 12 THEN '10-12' " +
                "WHEN ss.grade BETWEEN 12 AND 15 THEN '12-15' " +
                "WHEN ss.grade BETWEEN 15 AND 18 THEN '15-18' " +
        ...
        @Query("SELECT FUNCTION('TO_CHAR', ss.gradedAt, 'YYYY-MM') AS month, AVG(ss.grade) AS average " +
                "FROM EvaluationGrade ss " +
